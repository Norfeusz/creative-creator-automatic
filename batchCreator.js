...
// --- Funkcje pomocnicze (teraz przyjmują klucz API z argumentu) ---
...
async function runAutomation(record, apiKey) {
    ...
    try {
        ...
        if (!newFolderId) { return { success: false, message: 'Nie udało się utworzyć nowego folderu. Sprawdź, czy URL jest poprawny.' }; }
        ...
        if (creationResult) { return { success: true, message: `Kreacja "${creativeNameWithPrefix}" została pomyślnie utworzona!` }; } else { return { success: false, message: 'Nie udało się utworzyć kreacji. Upewnij się, że link docelowy jest poprawny.' }; }
    } catch (error) {
        if (error.message === "unauthorized") { return { success: false, message: "Podałeś nieprawidłowy API Key, lub nie masz uprawnień do dodawania kreacji." }; }
...
app.post('/verify-api-key', async (req, res) => {
    ...
    try {
        ...
        // Używamy prostego endpointu do weryfikacji klucza
